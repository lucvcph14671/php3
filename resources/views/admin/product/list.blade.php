        ...
        <tbody>
            @foreach ($db as $index => $product)
                <tr>
                    ...
                    @if ($product->status === 1)
                        <td >
                            <a class="btn btn-outline-danger" name="" href="{{route('admin.product.status', $product->id)}}">Khóa</a>
                            </td>
                    @elseif ($product->status === 0)
                        <td>
                            <a class="btn btn-outline-info" name="st" href="{{route('admin.product.status', $product->id)}}">Mở</a>
                           </td>
                    @endif
                    <td>
